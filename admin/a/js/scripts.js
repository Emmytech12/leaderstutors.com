...
function handleAdminClick() {
  ...
  if (login_role_id > 1) {
    ...
    adminDiv.removeAttribute('onclick');
    adminDiv.classList.add('fade-out');
  }
}
...
function _get_page_content(page, actid, ids) {
	if (page == '') {
		...
		$('#page_content, #page_pictures').removeClass('active-li');
		$('#' + actid).addClass('active-li');
		
			$('#get_details').html('<div class="ajax-loader"><img src="all-images/images/ajax-loader.gif"/></div>').fadeIn(500);
...
function _get_header_pix(profile_pix) {
  header_pix ='<img id="passportimg2" src="'+ staff_picture_directory +"/" + profile_pix +'" />';
...
        },
    success: function (info) {
      ...
      if (success == true) {
        ...
        $('#current_user_passport2').html('<img id="passportimg1" src="'+ staff_picture_directory +"/" + profile_pix +'"/>');
...
        },
    success: function (info) {
      ...
      if (success == true) {
        ...
        $('#last_login_time').html(last_login_time);
        $('#staff_status_name').html(status_name);
        $('#current_user_passport1').html('<img id="passportimg4" src="'+ staff_picture_directory +"/" + profile_pix +'"/>');
          ...
          if (!fetch || (Array.isArray(fetch) && fetch.length === 0)) {
            ...
            for (var i = 0; i < fetch.length; i++) {
              ...
              text +=
                ...
                '<div class="pix-div"><img src="' + staff_picture_directory + "/" + profile_pix + '"/></div>' +
                '<div class="detail">' +
                ...
                for (var i = 0; i < fetch.length; i++) {
                    ...
                    if (div_id2 == 'fetch_all_subjects') {
                        text +=
                            ...
                            '<div class="div-in">' +
                            '<div class="image-div"><img src="' + subjectPixPath + "/" + thumbnail + '" alt="' + subject_name + '""/></div>' +
...
        },
    success: function (info) {
      ...
      $("#updt_urls").val(urls);
      $("#updt_status_id").append('<option value="' + status_id +'" selected="selected">' + status_name +"</option>");
      $('#view_pix').html('<img src="'+ subjectPixPath +"/" + thumbnail +'" id="subject-pix" alt="'+ subject_name +'"/>');
          ...
          if (!fetch || (Array.isArray(fetch) && fetch.length === 0)) {
            ...
            if (div_id == "fetch_classes"){
              text +=
                '<div class="grid-div animated fadeIn">' +
                  '<div class="div-in">' +
                    '<div class="image-div"><img src="'+ classPixPath +"/" + thumbnail +'" alt="'+ class_name +'""/></div>'+
                    '<div class="ACTIVE '+status_name +'">' +status_name +'</div>'+
...
        },
    success: function (info) {
      ...
      $("#updt_urls").val(urls);
      $("#updt_status_id").append('<option value="' + status_id +'" selected="selected">' + status_name +"</option>");
      $('#view_pix').html('<img src="'+ classPixPath +"/" + thumbnail +'" id="subject-pix" alt="'+ class_name +'"/>');
    } else {
      var response = info.response;
      if(response<100){
        _logout();
          ...
          if (!fetch || (Array.isArray(fetch) && fetch.length === 0)) {
              ...
              text +=
                  '<div class="record-content-div animated fadeIn">'+
                    '<div class="div-in">'+
                        '<div class="image-div">'+
                            '<img src="'+ departmentPixPath +"/" + thumbnail +'" alt="'+ department_name +'""/>'+
                        '</div>'+
...
        },
    success: function (info) {
      ...
      $("#updt_urls").val(urls);
      $("#updt_status_id").append('<option value="' + status_id +'" selected="selected">' + status_name +"</option>");
      $('#view_pix').html('<img src="'+ departmentPixPath +"/" + thumbnail +'" id="subject-pix" alt="'+ department_name +'"/>');
    } else {
      var response = info.response;
      if(response<100){
        _logout();
        ...
        if (success == true) {
          ...
          text +=
              '<div class="record-content-div animated fadeIn">'+
                '<div class="div-in">'+
                    '<div class="image-div">'+
                        '<img src="'+ classPixPath +"/" + thumbnail +'" alt="'+ class_name +'""/>'+
                    '</div>'+
        ...
        if (success == true) {
          ...
          text +=
              '<div class="record-content-div animated fadeIn">'+
                '<div class="div-in">'+
                    '<div class="image-div">'+
                        '<img src="'+ subjectPixPath +"/" + thumbnail +'" alt="'+ subject_name +'""/>'+
                    '</div>'+
...
function _get_fetch_subject_form(subject_id) {
  ...
          },
      success: function (info) {
        ...
        $('#updt_subject_name').html(subject_name);
      },
    });
}
                ...
                for (var j = 0; j < week_videos.length; j++) {
                    ...
                    text +=
                     '<div class="topics-content-div">'+
                        '<div class="image-div"><img src="'+ tutorialPixPath +'/'+ thumbnail +'" alt="'+ topic +'"/>'+
                        '</div>'+
                        ...
                        '<div class="text">'+
                            ...
                            '<div class="bottom-div">'+
                                '<button class="btn edit" title="EDIT VIDEO" onClick="_get_form_with_id(' +"'update_video'" +"," +"'" + department_id +"'" +"," +"'" + class_id +"'" +"," +"'" + tutorial_id + "'" +');"><i class="bi-pencil-square"></i> EDIT</button>'+
                                '<button class="btn" title="VIEW CBT" onClick="_get_page_with_id();"> <span class="count">0</span> CBT </button>'+
  ...
  $.ajax({
    ...
    success: function (info) {
      ...
      if (success == true) {
        ...
        $('#updt_status_id').append('<option value="' + status_id +'" selected="selected">' + status_name +"</option>");
        $('#view_tutorial_pix').html('<img src="'+ tutorialPixPath +"/" + thumbnail +'" id="subject-pix" alt="'+ topic +'"/>');
        _get_tutorial_pdf(material);
        _get_tutorial_video(video);
      } else {
        $('#warning-div').html('<div><i class="bi-exclamation-octagon-fill"></i></div> <span>'+ message +'</span>').fadeIn(500).delay(5000).fadeOut(100);
      }
    },
...
function _get_tutorial_video(video) {
  var videoDisplay = document.getElementById('videoDisplay');
  // Set the source of the video element
  videoDisplay.src = tutorialVideoPath + "/" + video;
...
function _fetch_system_alert(view_to) {
  var search_keywords = $("#search_keywords").val();
  ...
  if (search_keywords.length > 2 || search_keywords == '') {
    var dataString = 'search_keywords=' + search_keywords + '&view_to=' + view_to; 
            ...
            for (var i = 0; i < fetch.length; i++) {
              ...
              if (seen_status==0){
                  ...
                  '<div class="system-alert alert-seen" id="'+ alert_id +'" onclick="_get_form_with_id(' + "'alert-read'" + "," + "'" + alert_id + "'" + ')">' +
                    '<div class="alert-name"><i class="bi-person"></i> ' + user_name + '<span id="'+ alert_id +'viewed"><i class="bi-check"></i><i class="bi-check"></i></span></div>' +
                    '<div class="alert-text">Success Alert: ' + alert_detail + '...</div>' +
                    '<div class="alert-time"><i class="bi-clock"></i> <span>' + created_time + '</span></div>' +
                  '</div>';
              }
...
function _fetch_next_alerts() {
  var view_from = parseInt($('#view_from').text()); // Retrieve the current view_from value
  var next_view_from = view_from + 51; // Increment view_from by 51
  var next_view_to = next_view_from + 95; // Calculate the corresponding view_to
  $('#view_from').text(next_view_from); // Update the view_from displayed on the page
  _fetch_system_alert(next_view_to); // Fetch alerts using the updated view_to
}
...
// function _fetch_next_alerts() {
//   var view_to = parseInt($('#view_to').text()); // Retrieve the current view_from value
//   var next_view_from = view_to + 50; // Increment view_from by 50
//   $('#view_to').text(next_view_from); // Update the view_from displayed on the page
//   _fetch_system_alert(next_view_from); // Fetch alerts using the updated view_from
// }
...
function _fetch_previous_alerts() {
  var view_from = parseInt($('#view_from').text());
  var previous_view_from = view_from - 50; 
  $('#view_from').text(previous_view_from);
  _fetch_system_alert(previous_view_from); 
}
